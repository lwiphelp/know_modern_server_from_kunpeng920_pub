...
务器，我们也只有一个这样的对象。所以，大部分是后我们谈PCIE的IOV功能，都是指
SR-IOV功能。
...
PCIe允许EP中Cache这个被翻译的地址，这个Cache称为ATC，Address Translation Client
。它需要响应RC的Cache Invalidate等命令。

我们要注意到，ATS是一个服务，而不是地址访问代理，所以实际上一个非法的EP完全可以
发出一个物理地址，说自己有这个能力。所以真正起拒绝作用的是RC的设计，而且这只能
保护内存，而不能保护网桥中的其他硬件设备。(todo：请硬件的通讯确认一下这个判断）

鲲鹏920使用ARM SMMU标准来完成这个实现，ATS服务由SMMU单元响应，而最终送到RC的内存
地址请求，也由SMMU作最后的确认，从而防止非法的硬件攻击计算子系统。
